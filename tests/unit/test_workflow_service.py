...
def test_store_persists_and_queues_due_runs(tmp_path: Path) -> None:
    ...
    created = store.upsert_workflow(workflow)
    assert created.definition_status == "valid"
    assert created.next_run_at is not None
    assert store.get_workflow(workflow.id).name == "Interactive Reveal Capture"
...
def test_lazy_scroll_workflow_manifest_exists() -> None:
    workflow_path = REPO_ROOT / "tasks" / "lazy_scroll_capture" / "workflow.toml"
    payload = workflow_path.read_text(encoding="utf-8")
    assert "interval_seconds = 900" in payload
