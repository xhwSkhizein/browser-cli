...
def normalize_schedule(
    ...
    *,
    ...
    try:
        ...
        raise InvalidInputError(f"Invalid automation timezone: {timezone_name}") from exc
    ...
    if normalized_kind == "interval":
        ...
        if seconds <= 0:
            raise InvalidInputError("Interval automations require positive interval_seconds.")
    ...
    if normalized_kind == "weekly":
        ...
        if weekday not in WEEKDAY_NAMES:
            ...
            raise InvalidInputError(f"Weekly automations require weekday in {{{expected}}}.")
        _validate_hour_minute(hour, minute)
        return normalized_kind, {"weekday": weekday, "hour": hour, "minute": minute}, timezone_name
    raise InvalidInputError(f"Unsupported automation schedule mode: {normalized_kind}")
...
def _validate_hour_minute(hour: int, minute: int) -> None:
    if hour < 0 or hour > 23:
        raise InvalidInputError("automation schedule hour must be between 0 and 23.")
    if minute < 0 or minute > 59:
        raise InvalidInputError("automation schedule minute must be between 0 and 59.")
