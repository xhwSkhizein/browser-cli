...
from browser_cli.workflow.scheduler import compute_next_run_at, normalize_schedule
...
class WorkflowStore:
    def __init__(self, db_path: Path | None = None) -> None:
        self._db_path = db_path or get_app_paths().workflow_db_path
        ...
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_workflow_runs_status
                    ON workflow_runs(status, queued_at ASC);
                ...
                CREATE INDEX IF NOT EXISTS idx_workflow_run_events_run_id
                    ON workflow_run_events(run_id, created_at ASC);
                """
            )
    ...
    def upsert_workflow(self, workflow: PersistedWorkflowDefinition) -> PersistedWorkflowDefinition:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                """,
                _workflow_to_row(updated),
            )
        return self.get_workflow(updated.id)

    def _existing_created_at(self, workflow_id: str) -> str | None:
        with self._connect() as conn:
            row = conn.execute(
    ...
    def get_workflow(self, workflow_id: str) -> PersistedWorkflowDefinition:
        with self._connect() as conn:
    ...
    def list_workflows(self) -> list[PersistedWorkflowDefinition]:
        with self._connect() as conn:
            rows = conn.execute("SELECT * FROM workflows ORDER BY name, id").fetchall()
        return [_row_to_workflow(row) for row in rows]

    def set_enabled(self, workflow_id: str, enabled: bool) -> PersistedWorkflowDefinition:
        workflow = self.get_workflow(workflow_id)
        updated = replace(workflow, enabled=enabled)
        return self.upsert_workflow(updated)
    ...
    def create_run(
        ...
        *,
        ...
        workflow = self.get_workflow(workflow_id)
        if workflow.definition_status != "valid":
            raise ValueError(
                f"Workflow {workflow_id} is invalid: {workflow.definition_error or 'invalid'}"
            )
        now = _utcnow()
        run = WorkflowRunRecord(
            ...
            status="queued",
            effective_inputs=dict(effective_inputs or workflow.input_overrides),
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT INTO workflow_runs (
                    run_id, workflow_id, trigger_type, status, effective_inputs_json,
    ...
    def enqueue_due_runs(self) -> list[str]:
        ...
        for workflow in self.list_workflows():
            if (
                ...
                or workflow.definition_status != "valid"
                or not workflow.next_run_at
            ):
                continue
            if workflow.schedule_kind == "manual":
                continue
            if workflow.next_run_at > now:
                continue
            if self._has_pending_run(workflow.id):
                continue
            run = self.create_run(workflow.id, trigger_type="scheduled")
            queued.append(run.run_id)
            self._update_next_run(workflow.id)
        return queued

    def _has_pending_run(self, workflow_id: str) -> bool:
        with self._connect() as conn:
            row = conn.execute(
                """
    ...
    def _update_next_run(self, workflow_id: str) -> None:
        workflow = self.get_workflow(workflow_id)
        normalized = self._normalize_workflow(workflow)
        with self._connect() as conn:
            conn.execute(
                "UPDATE workflows SET next_run_at = ?, updated_at = ? WHERE id = ?",
                (normalized.next_run_at, _utcnow(), workflow_id),
            )

    def claim_next_run(self) -> WorkflowRunRecord | None:
        with self._connect() as conn:
    ...
    def complete_run(
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            row = conn.execute(
                "SELECT workflow_id FROM workflow_runs WHERE run_id = ?", (run_id,)
            ).fetchone()
            if row is not None and status == "success":
                conn.execute(
                    ...
                    WHERE id = ?
                    """,
                    (finished_at, finished_at, str(row["workflow_id"])),
                )
    ...
    def add_run_event(self, run_id: str, event: WorkflowRunEvent) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO workflow_run_events (run_id, event_type, message, created_at, payload_json)
                VALUES (?, ?, ?, ?, ?)
                """,
                (
                    run_id,
    ...
    def retry_run(self, run_id: str) -> WorkflowRunRecord:
    ...
    def service_metrics(self) -> dict[str, Any]:
        with self._connect() as conn:
            queued = conn.execute(
                "SELECT COUNT(*) AS count FROM workflow_runs WHERE status = 'queued'"
            ).fetchone()
            running = conn.execute(
                "SELECT COUNT(*) AS count FROM workflow_runs WHERE status = 'running'"
            ).fetchone()
            workflows = conn.execute("SELECT COUNT(*) AS count FROM workflows").fetchone()
        return {
            "workflow_count": int(workflows["count"]),
    ...
    def _normalize_workflow(
        ...
        task_path = workflow.task_path.expanduser().resolve()
        task_meta_path = workflow.task_meta_path.expanduser().resolve()
        output_dir = (
            workflow.output_dir.expanduser().resolve()
            if str(workflow.output_dir)
            else (get_app_paths().workflow_runs_dir / workflow.id).resolve()
        )
        result_json_path = None
        if workflow.result_json_path is not None:
            result_json_path = workflow.result_json_path.expanduser()
        ...
        return replace(
            ...
            next_run_at=next_run_at if workflow.enabled else None,
        )


def _workflow_to_row(workflow: PersistedWorkflowDefinition) -> dict[str, Any]:
    return {
        "id": workflow.id,
        "name": workflow.name,
        ...
        "enabled": 1 if workflow.enabled else 0,
        ...
        "schedule_payload_json": json.dumps(
            workflow.schedule_payload, ensure_ascii=False, sort_keys=True
        ),
        "timezone": workflow.timezone,
        "output_dir": str(workflow.output_dir),
        "result_json_path": str(workflow.result_json_path) if workflow.result_json_path else None,
        "stdout_mode": workflow.stdout_mode,
        "input_overrides_json": json.dumps(
            workflow.input_overrides, ensure_ascii=False, sort_keys=True
        ),
        "before_run_hooks_json": json.dumps(list(workflow.before_run_hooks), ensure_ascii=False),
        "after_success_hooks_json": json.dumps(
            list(workflow.after_success_hooks), ensure_ascii=False
        ),
        "after_failure_hooks_json": json.dumps(
            list(workflow.after_failure_hooks), ensure_ascii=False
        ),
...
def _row_to_workflow(row: sqlite3.Row) -> PersistedWorkflowDefinition:
...
def _run_to_row(run: WorkflowRunRecord) -> dict[str, Any]:
    return {
        "run_id": run.run_id,
...
def _row_to_run(row: sqlite3.Row) -> WorkflowRunRecord:
...
def _row_to_event(row: sqlite3.Row) -> WorkflowRunEvent:
